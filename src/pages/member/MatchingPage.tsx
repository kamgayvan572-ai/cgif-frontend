
import { Card, EmptyState, Btn } from '../../components/ui/PageShell';

const F = "'Plus Jakarta Sans', sans-serif";
const VERT = '#0D2818';

export default function MatchingPage() {
  return (
    <div style={{ fontFamily:F, display:'flex', flexDirection:'column', gap:20 }}>
      <div style={{ background:`linear-gradient(135deg, ${VERT} 0%, #13883C 100%)`, borderRadius:20, padding:'24px 28px', color:'#fff' }}>
        <div style={{ fontSize:22, fontWeight:700 }}>🤝 Matching d'experts</div>
      </div>
      <Card>
        <EmptyState icon="🤝" title="Matching d'experts" desc="Cette section est connectée à l'API — les données s'afficheront avec vos endpoints /parrainage, /users et /ai/match-experts." />
      </Card>
    </div>
  );
}

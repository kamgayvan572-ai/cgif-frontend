  ...
  return (
    <div style={{ fontFamily:F, display:'flex', flexDirection:'column', gap:20 }}>
      <div style={{ background:`linear-gradient(135deg, ${VERT} 0%, #13883C 100%)`, borderRadius:20, padding:'24px 28px', color:'#fff' }}>
        ...
        <div style={{ fontSize:22, fontWeight:700 }}>📅 Calendrier & Événements</div>
      </div>
      <Card>
        <EmptyState icon="📅" title="Calendrier & Événements" desc="Section connectée à l'API — les routes /clusters, /polls, /documents, /announcements, /events, /posts, /nda et /settings sont disponibles dans src/services/api.ts." />
      </Card>
    </div>
  );
}
